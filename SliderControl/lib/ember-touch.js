  ...
  _invokeEvent: function(eventName, eventObject) {
    ...
    if ( !Em.AppGestureManager.get('isAllBlocked') ) {
      ...
      if ( parentView ) {
        ...
        if ( manager && !eventObject.isPropagationStopped()) { manager._invokeEvent(eventName, eventObject); }//Added condition !eventObject.isPropagationStopped() to propagate events only when propagation is not stopped.
      }
